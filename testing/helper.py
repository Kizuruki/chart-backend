...
class _RoutedFunction(Protocol):
    ...
    ) -> Generator[Optional[Body], requests.Response, Any]: ...


class After:
    def __init__(
        self,
        after: _RoutedFunction,
...
class _Route:
    def __init__(
        self, path: str, func: _RoutedFunction, method: str, dependencies: list[After]
    ):
        self.path = path
        self.func = func
        self.method = method
...
class Test:
    ...
    def start(self):
            ...
            try:
                for dependency in route.dependencies:
                    if dependency.id not in self.processed_routes:
                        print("\n")
                        raise Exception(
                            f"Dependency {dependency.id} is not yet processed"
                        )

                    if dependency.id in self.failed_routes:
                        print(
                            Fore.YELLOW
                            + f"SKIPPED | Dependency {dependency.id} failed, skipping this route"
                        )
                        raise SkipRoute()
                    ...
                    if dependency.value or dependency.use_for_auth:
                        if dependency.id not in ret_vals:
                            raise Exception(
                                f'Dependency {dependency.id} returned nothing, but function requires value "{dependency.value if dependency.value else _EMPTY_STRING}"'
                            )
                ...
                try:
                    ...
                    if body.use_private_auth:
                        headers[config["server"]["auth-header"]] = config["server"][
                            "auth"
                        ]
                    elif auth:
                    ...
                    if not response.ok:
                        print(
                            Fore.RED
                            + f"FAILED | Request status {response.status_code}, text: {response.text}"
                        )
        ...
        print(
            f"{len(self.failed_routes)}/{len(self.routes)} ({round(len(self.failed_routes) / len(self.routes) * 100, 2)}%) failed, {skipped_routes} of them were skipped"
        )
