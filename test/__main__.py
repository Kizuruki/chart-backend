...
USE_FAKE_EXTERNAL_AUTH = not input(
...
if USE_FAKE_EXTERNAL_AUTH:
    ...
    @test.route(
        "/accounts/session/external/",
        "POST",
        dependencies=[After(external_auth_step1, value="external_auth_id")],
    )
    ...
    @test.route(
        "/accounts/session/external/get/",
        "GET",
        dependencies=[After(external_auth_step1, value="external_auth_id")],
    )
    def external_auth(external_auth_id: str):
        print(
            f"Auth at https://open.sonolus.com/external-login/{test.sonolus_url}?id={external_auth_id} and press enter to continue",
            end=" ",
        )
        input()
...
@test.route(
    ...
    "GET",
    dependencies=[After(external_auth, use_for_auth=True)],
)
def account():
    response: Response = yield
    yield response.json()
...
@test.route(
    "/accounts/{id}/staff/unadmin",
    "PATCH",
    dependencies=[After(account, value="account"), After(admin)],
)
...
@test.route(
    "/accounts/{id}/staff/mod", "PATCH", dependencies=[After(account, value="account")]
)
...
@test.route(
    ...
    "POST",
    dependencies=[
        After(external_auth, use_for_auth=True),
        After(account, value="account"),
        After(mod),
    ],
)
def add_notification(account: dict):
    yield Body(
        data={"user_id": account["id"], "title": "notification.", "content": "test"}
    )
...
@test.route(
    ...
    "GET",
    dependencies=[After(external_auth, use_for_auth=True), After(add_notification)],
)
...
@test.route(
    ...
    dependencies=[
        After(external_auth, use_for_auth=True),
        After(get_notification_list, value="id"),
    ],
)
...
@test.route(
    "/charts/upload/", "POST", dependencies=[After(external_auth, use_for_auth=True)]
)
def upload_chart():
    response: Response = yield Body(
        data={
        ...
        files={
            "jacket_image": (
                "jacket.png",
                open("assets/jacket.png", "rb"),
                "image/png",
            ),
            "chart_file": (
                "chart.sus",
                open("assets/level.data", "rb"),
                "application/gzip",
            ),
...
@test.route(
    "/accounts/session/", "POST", dependencies=[After(account, value="account")]
)
...
@test.route(
    "/charts/{chart_id}/comment/{comment_id}/",
    "DELETE",
    dependencies=[
        After(game_auth, use_for_auth=True),
        After(upload_chart, value="chart_id"),
        After(get_comments, value="comment_id"),
    ],
)
